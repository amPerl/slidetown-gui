...
impl Mesh {
    ...
    fn visit_ni_tri_shape(
        ...
        for property_ref in ni_tri_shape.property_refs.iter().filter(|r| r.0 >= 0) {
            let property = property_ref
                .get(&nif.blocks)
                .with_context(|| format!("Invalid property ref: {:?}", property_ref))?;
            if let Block::NiStencilProperty(_) = property {
                eprintln!("skipping ni_tri_shape, found NiStencilProperty");
                return Ok(());
            }
        }
